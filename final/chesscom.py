...
def get_top_players_info():
    ...
    for i in range(0, len(all)):
        ...
        info = [
            ...
            res.player.url,
            res.player.username,
            res.player.followers,
            res.player.country,
            datetime.fromtimestamp(res.player.joined),
            res.player.is_streamer,
            res.player.verified,
            fide,
        ]
...
def get_country_clubs_data(country_code):
    print("chesscom::get_country_clubs_data::country_code: " + country_code)
    ...
    for c in clubs:
        ...
        info = [
            ...
            res.club.name,
            res.club.club_id,
            res.club.average_daily_rating,
            res.club.members_count,
            datetime.fromtimestamp(res.club.created),
        ]
...
def process_chesscom(countryCode):
    print("chesscom::process_chesscom::countryCode" + countryCode)
    country_clubs_data = get_country_clubs_data(
        countryCode
    )  # TODO: Parametrizar el pais de los clubs a estudiar
